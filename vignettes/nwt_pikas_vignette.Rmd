...
Not only do we have stress measurements, but the `utm_easting` and `utm_northing` columns also provide the geographic coordinates of the fecal pellet sampling stations where the pikas were found by researchers. 
...
We now have a geometry column consisting of points instead of two separate columns for our coordinates. However, there is still no Coordinate Reference System (CRS) associated with our points. According to the University of Colorado Boulder's [Earth Lab](https://www.earthdatascience.org/courses/earth-analytics/spatial-data-r/intro-to-coordinate-reference-systems/), a Coordinate Reference System describes several things about your data: how the Earth’s 3D surface is represented in a 2D form (spatial projection), how the Earth's shape is modeled (datum), the units, and the axes. 

From the package documentation, which can be viewed by running `?nwt_pikas`, we can see that our measurements use the Universal Transverse Mercator (UTM) system, are in Zone 13, and use the NAD83 datum. These all describe the CRS that our data uses.

We can associate these fields with the geometry column using `st_set_crs()`. This can be done in two ways. You can either use a string to describe the coordinate system using PROJ4 syntax:

```{r proj4, eval=FALSE}
pikas_sf <-
  pikas_sf %>% mutate(geometry = st_set_crs(geometry, "+proj=utm +zone=13 +datum=NAD83 +units=m"))
```
...
```{r epsg}
pikas_sf <-
  pikas_sf %>% mutate(geometry = st_set_crs(geometry, 26913))
```
...
In the future, we may want to plot our data on top of a web-based map. Online maps often use the WGS84 Web Mercator coordinate system, which uses latitude and longitude coordinates. After setting our CRS, we can reproject our data to the WGS84 CRS using `st_transform()` so that it's more compatible with third-party maps, which will give us more ways to represent our data:

PROJ4 Syntax:

```{r, eval=FALSE}
pikas_sf <-
  pikas_sf %>% mutate(geometry = st_transform(geometry, "+proj=longlat +datum=WGS84"))
```
...
```{r}
pikas_sf <-
  pikas_sf %>% mutate(geometry = st_transform(geometry, 4326))
```
...
Because we reprojected our data, we can now plot our data onto geographic maps. We can use `ggplot`'s `map_data()` function to plot our data with basic maps from the `maps` package. First, let's plot our data on top of a map of the United States:

```{r}
usa <- map_data("usa")
usa_plot <- ggplot() + 
  geom_polygon(data = usa, aes(x=long, y =lat, group=group)) + 
  geom_sf(data = pikas_sf, aes(color = site, shape = site), alpha = 0.6) +
  theme_minimal() + 
  labs(title = "Location of Pika Sampling Stations in the USA", subtitle = "Niwot Ridge LTER", x = "Latitude (Degrees)", y = "Longitude (Degrees)") 

usa_plot
```

Next, we can plot our data onto a county map of Colorado to get a closer look:

```{r}
colorado <- map_data("county", "colorado")
colorado_plot <- ggplot() + 
  geom_polygon(data=colorado, aes(x=long, y =lat, group=group), color="white") +
  geom_sf(data = pikas_sf, aes(color = site, shape = site), alpha = 0.6) + 
  theme_minimal() + 
  labs(title = "Location of Pika Sampling Stations in Colorado", subtitle = "Niwot Ridge LTER", x = "Latitude (Degrees)", y = "Longitude (Degrees)") 
...
combined_map <- ggmap(get_stamenmap(nwt, zoom = 10, maptype = "terrain")) + 
 geom_sf(data = pikas_sf, inherit.aes = FALSE, aes(color = site, shape = site), alpha = 0.3) +
  theme_minimal() +
...
You can also use the `leaflet` package to produce interactive plots:

```{r leaflet}
leaflet() %>% 
  addProviderTiles("Stamen.Terrain") %>% 
  addCircles(data = pikas_sf)
```

You can learn more about the `leaflet` package [here](https://rstudio.github.io/leaflet/). There is also a cheatsheet available [here](https://github.com/rstudio/cheatsheets/blob/master/leaflet.pdf).
