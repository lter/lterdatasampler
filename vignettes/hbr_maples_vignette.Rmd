...
The `hbr_maples` data sample is in tidy format and contains both numeric and factor variables. Learn more about the variables by checking the dataset documentation (`?hbr_maples`). 

```{r}
glimpse(hbr_maples)
```

Now, let's create an exploratory visualization of the data using `ggplot`. Is there evidence for differences in maple seedling height (millimeters) between the calcium-treated (W1) and untreated (reference) watersheds?
...
In both years of the study (2003 and 2004) it appears that seedling height is greater overall in the calcium-treated watershed (W1), compared to the untreated watershed.
...
In both years, the mean and median seedling height (stem_length) are higher in the W1 (calcium-treated) watershed, compared to the untreated watershed. 
...
Having explored distributions in the histograms and QQ plots above, we can conclude that the data are approximately normal. As a next step, we will use `var.test()` to test for equal variances of 2004 seedling height (`stem_length`, in millimeters) observations in each watershed. 
...
By default, `t.test()` will use Welch's test to correct for unequal variances, so we set `var.equal = TRUE` based on the outcome of our F-test above:
