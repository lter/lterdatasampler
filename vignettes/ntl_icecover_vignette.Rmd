...
There is an upward trend in the mean annual temperature. Let's investigate potentialrelationships between air temperature and lake ice cover duration.
...
ntl_airtemp_avg_winter <-  ntl_airtemp_hydro %>%
  filter(month(sampledate) %in% c(11:12,1:4)) %>%
  group_by(hydroyear) %>%
...
There is a negative correlation between the mean air temperature from December to April and the amount of time that each lake was frozen. This means that as temperature increases, the ice duration decreases.
...
ntl_airtemp_clim <- ntl_airtemp_avg_winter %>%
  filter(hydroyear > 1900) %>%   # The rest of the timeseries
  add_column(clim_winter_mean_airtemp = ntl_airtemp_clim_30first$clim_winter_mean_airtemp,
             clim_winter_sd_airtemp = ntl_iceduration_clim_30first$clim_sd_iceduration) %>%
...
temp_plot_airtemp_anomalies <-
  ...
  geom_bar(data = stat = "identity", show.legend = FALSE) +
  theme_minimal() +
...
ntl_iceduration_clim <- ntl_icecover %>%
  filter(year > 1900) %>%   # The rest of the timeseries
...
Note the difference of scale on the y-axis. However in both time series, there is an increase in anomalies (bith in frequency and intensity) that is increasing over time
